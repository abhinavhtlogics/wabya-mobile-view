...
const Dashboard = () => {
  ...
  const [clientCaluserName, setclientCaluserName] = useState('');
  ...
  const scheduleNext = async() => {
   ...
   setbookingError(false);
    ...
    "customInputs":[]
      ...
      try {
        const res = await fetch('https://api.cal.com/v1/bookings?apiKey='+coachesCalApiKey+'', {
        method: 'POST',
        headers: {
  ...
  const getBookingId = async() => {
      ...
      try {
        const res = await fetch('https://api.cal.com/v1/bookings/'+BookedId+'?apiKey='+coachesCalApiKey+'', {
        method: 'GET',
        headers: {
          'Content-Type': 'application/json',
        }
...
addDoc(meetRef, {
  ...
  const eventChooseClick = (event) => {
    ...
    setcoachesCalUsername(event.target.getAttribute('data-coach_username'));

    console.log(event.target.getAttribute('data-coaches_api'));
    setcoachesCalApiKey(event.target.getAttribute('data-coaches_api'));
  ...
  useEffect(() => {
    if(client != null){
      ...
      setclientCalAPIkey(client.client_api);
  ...
  const getCoachData = async () => {
    await getDocs(coachRef).then(response => {
      setCoachData(
        response.docs.map(data => {
  ...
  const callAPI = async date => {
    ...
    try {
      const res = await fetch(`https://api.cal.com/v1/schedules/63415?apiKey=cal_live_68009c870708ee42ae839248d427f6eb`)
      const data = await res.json()
      console.log(data)
  ...
  const getEventTypes = async() => {
    ...
    try {
      const res = await fetch('https://api.cal.com/v1/event-types?apiKey='+coachesCalApiKey+'', {
      method: 'GET',
      headers: {
        'Content-Type': 'application/json',
      }
 ...
 const getUpcomingBooking = async() => {
  ...
  try {
    const res = await fetch('https://api.cal.com/v1/bookings?apiKey='+clientCalAPIkey+'', {
    method: 'GET',
    headers: {
      'Content-Type': 'application/json',
    }
...
// setNext(true);
  ...
  const getUsers = async() => {
    ...
    try {
      const res = await fetch('https://api.cal.com/v1/users?apiKey='+coachesCalApiKey+'', {
      method: 'GET',
      headers: {
        'Content-Type': 'application/json',
      }
 ...
 // setNext(true);
  ...
  const getMyDetailFromCal = async() => {
    ...
    try {
      const res = await fetch('https://api.cal.com/v1/users?apiKey='+clientCalAPIkey+'', {
      method: 'GET',
      headers: {
        'Content-Type': 'application/json',
      }
...
const getTimeslots = async date=> {
  ...
  setarray1([]);
    ...
    try {
      const res = await fetch('https://api.cal.com/v1/schedules/'+scheduleId+'?apiKey='+coachesCalApiKey+'', {
      method: 'GET',
      headers: {
        'Content-Type': 'application/json',
      }
...
const getBookedSchedule = async() => {
  ...
  var busySchedule=[];
    ...
    try {
      const res = await fetch('https://api.cal.com/v1/availability?apiKey='+coachesCalApiKey+'&dateFrom='+dateFrom+'&dateTo='+dateTo+'&username='+coachesCalUsername+'', {
      method: 'GET',
      headers: {
        'Content-Type': 'application/json',
      }
                          ...
                          return (
                            <>
                              <tr>
                                ...
                                <td>
                                  <button className='btn btn-choose'  data-coaches_name={data.coach_name} data-coaches_id={data.id} data-coaches_api={data.coach_api} data-coach_username={data.coach_uname} onClick={eventChooseClick}>
                                    Choose
                                  </button>
                                </td>
                              </tr>
              ...
              <div className='banner-et'>
                ...
                <table className='table table-coach'>
                  <tbody>
                    ...
                    {upcomingMetting.map((upcomingmeet, index) => {

                      return (

                        Date.parse(upcomingmeet.startTime) > Date.parse(currentdate)?

                        upcomingmeet.status=='ACCEPTED'?
                    <tr className='table-pad'>
                      <td>
                        <span>{upcomingmeet.user.name} </span>
                      </td>
                      <td>
                        <span>{new Date(upcomingmeet.startTime).toLocaleDateString()} </span>
                      </td>
                      <td>
<a href={`${upcomingmeet.metadata.videoCallUrl}`} className='btn' target='_blank'>Join Video</a>
                      </td>
                      <td>
                        <button className='btn btn-schedule' onClick={rescheduleSession}>
